...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/cogynt/cogynt-prometheus-pushgateway"  
  }
}
...
module "workload_identities_cogynt_prometheus_pushgateway" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "cogynt-loki"
  namespace           = "loki"
  use_existing_k8s_sa = true
  k8s_sa_name         = "cogynt-prometheus-pushgateway"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_cogynt_prometheus_pushgateway
}
