...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/px-prometheus-operator" 
  }
}
...
module "workload_identities_px_prometheus_operator" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "px-prometheus-operator"
  namespace           = "kube-system"
  use_existing_k8s_sa = true
  k8s_sa_name         = "px-prometheus-operator"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_px_prometheus_operator
}
