...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/linkerd-viz/tap-injector" 
  }
}
...
module "workload_identities_tap_injector" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "linkerd-viz"
  namespace           = "linkerd-viz"
  use_existing_k8s_sa = true
  k8s_sa_name         = "tap-injector"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_tap_injector
}
