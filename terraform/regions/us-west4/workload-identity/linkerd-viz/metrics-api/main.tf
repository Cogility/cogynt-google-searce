...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/linkerd-viz/metrics-api" 
  }
}
...
module "workload_identities_metrics_api" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "linkerd-viz"
  namespace           = "linkerd-viz"
  use_existing_k8s_sa = true
  k8s_sa_name         = "metrics-api"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_metrics_api
}
