...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/hcvault-agent-injector"  
  }
}
...
module "workload_identities_hcvault_agent_injector" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "hcvault-sa"
  namespace           = "kube-system"
  use_existing_k8s_sa = true
  k8s_sa_name         = "hcvault-agent-injector"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_hcvault_agent_injector
}
