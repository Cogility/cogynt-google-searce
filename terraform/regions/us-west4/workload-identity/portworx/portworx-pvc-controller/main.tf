...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/portworx/portworx-pvc-controller"  
  }
}
...
module "workload_identities_portworx_pvc_controller" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "portworx"
  namespace           = "kube-system"
  use_existing_k8s_sa = true
  k8s_sa_name         = "portworx-pvc-controller"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_portworx_pvc_controller
}
