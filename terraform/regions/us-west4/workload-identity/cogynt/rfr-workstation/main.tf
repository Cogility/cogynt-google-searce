...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/cogynt/rfr-workstation" 
  }
}
...
module "workload_identities_rfr_workstation" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "cogynt"
  namespace           = "cogynt"
  use_existing_k8s_sa = true
  k8s_sa_name         = "rfr-workstation"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_rfr_workstation
}
