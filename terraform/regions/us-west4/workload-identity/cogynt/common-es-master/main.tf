...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/cogynt/common-es-master" 
  }
}
...
module "workload_identities_common_es_master" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "cogynt"
  namespace           = "cogynt"
  use_existing_k8s_sa = true
  k8s_sa_name         = "common-es-master"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_common_es_master
}
