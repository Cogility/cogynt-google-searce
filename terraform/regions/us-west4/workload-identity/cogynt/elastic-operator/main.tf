...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/cogynt/elastic-operator" 
  }
}
...
module "workload_identities_elastic_operator" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "cogynt"
  namespace           = "cogynt"
  use_existing_k8s_sa = true
  k8s_sa_name         = "elastic-operator"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_elastic_operator
}
