...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/linkerd/linkerd-proxy-injector" 
  }
}
...
module "workload_identities_linkerd_proxy_injector" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "linkerd"
  namespace           = "linkerd"
  use_existing_k8s_sa = true
  k8s_sa_name         = "linkerd-proxy-injector"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_linkerd_proxy_injector
}
