...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/linkerd/linkerd-heartbeat" 
  }
}
...
module "workload_identities_linkerd_heartbeat" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "linkerd"
  namespace           = "linkerd"
  use_existing_k8s_sa = true
  k8s_sa_name         = "linkerd-heartbeat"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_linkerd_heartbeat
}
