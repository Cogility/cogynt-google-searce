...
terraform {
  backend "gcs" {
    ...
    prefix = "workload-identity/linkerd/linkerd-destination" 
  }
}
...
module "workload_identities_linkerd_destination" {
  source              = "terraform-google-modules/kubernetes-engine/google//modules/workload-identity" 
  project_id          = var.project_id
  name                = "linkerd"
  namespace           = "linkerd"
  use_existing_k8s_sa = true
  k8s_sa_name         = "linkerd-destination"
...
output "workload_identities" {
  ...
  value       = module.workload_identities_linkerd_destination
}
